...
namespace BigChange.MassTransit.AwsKeyManagementService.Tests
{
    ...
    public class KmsSecureKeyProviderTests
    {
        ...
        public void ShouldReturnKeyAndSetHeaderOnReceive()
        {
            ...
            var kmsSecureKeyProvider = new KmsSecureKeyProvider(amazonKeyManagementService.Object,
                encryptionContextBuilder.Object, keyId);
        ...
        public void ShouldReturnKeyAndSetHeaderOnSend()
        {
            ...
            bytes.ShouldBe(key);
            sendHeader.Verify(
                x => x.Set(KmsSecureKeyProvider.DataKeyCiphertextHeader, Convert.ToBase64String(keyCiphertext)),
                Times.Once);
        }
    }
}